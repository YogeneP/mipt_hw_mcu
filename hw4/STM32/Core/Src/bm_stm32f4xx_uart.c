...
uint32_t BM_UART_Init(uint64_t baud_rate) {
 ...
 // BM_UART_INST->BRR = ((uint64_t)BM_UART_BUS_CLOCK + ((uint64_t)baud_rate / 2ULL)) / baud_rate;
  ...
  // All the bits 0 state are good for me: USART_CR1_M, USART_CR1_PCE, 
...
{
  if((BM_UART_INST->SR & USART_SR_RXNE) && (BM_UART_INST->CR1 & USART_CR1_RXNEIE)) {
    CLEAR_BIT(BM_UART_INST->CR1, USART_CR1_RXNEIE);
    BM_UART_Receive(NULL,1); //repetative call: buf address NULL; len - non zero
  }
  if((BM_UART_INST->SR & USART_SR_TXE) && (BM_UART_INST->CR1 & USART_CR1_TXEIE)) {
